...
export const KIRO_BALANCE_ROW_ID = 'kiro:balance';
...
export function collectQuotaRowInstants(
  ...
  if (provider === 'kiro') {
    const nextResetAt = (quota as { nextResetAt?: number | null }).nextResetAt;
    if (!isUsableMs(nextResetAt) || nextResetAt <= 0) return [];
    const atMs = nextResetAt < 1_000_000_000_000 ? nextResetAt * 1000 : nextResetAt;
    return [{ rowId: KIRO_BALANCE_ROW_ID, atMs, kind: 'window' }];
  }
