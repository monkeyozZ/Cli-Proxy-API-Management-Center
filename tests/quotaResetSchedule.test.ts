...
describe('collectQuotaRowInstants', () => {
  ...
  test('converts the Kiro Unix-seconds reset to milliseconds', () => {
    const resetAtMs = NOW + 2 * HOUR_MS;
    const quota = { status: 'success', nextResetAt: resetAtMs / 1000 };
    expect(collectQuotaRowInstants('kiro', quota)).toEqual([
      { rowId: KIRO_BALANCE_ROW_ID, atMs: resetAtMs, kind: 'window' },
    ]);
  });
...
describe('nextRecoveryMs', () => {
  ...
  test('uses the Kiro balance reset as its recovery instant', () => {
    const resetAtMs = NOW + DAY_MS;
    expect(nextRecoveryMs('kiro', { status: 'success', nextResetAt: resetAtMs / 1000 }, NOW)).toBe(
      resetAtMs
    );
  });
